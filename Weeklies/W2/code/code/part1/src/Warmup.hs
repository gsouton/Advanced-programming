...
instance Monad RWSP where
  return a = RWSP (\read_data state_data -> (a, mempty, state_data))
  m >>= f =
    RWSP
      ( \r s ->
          let (a, w0, s0) = runRWSP m r s
           in let (b, w1, s1) = runRWSP (f a) r s0
               in (b, w0 <> w1, s1)
      )
...
withP r' m = RWSP (\_ state_data -> runRWSP m r' state_data)
...
askE :: RWSE ReadData
askE = RWSE (\r s -> Right (r, mempty, s)) -- freebie
